...
function BasketComponent(){
    ...
    const getAll = async() =>{
        ...
        let response = await axios.post("http://localhost:3840/baskets/getAll",model);
    ...
    const remove = async(_id) => {
        ...
        if(confirm){
        let model = {_id: _id};
        await axios.post("http://localhost:3840/baskets/remove",model);
        getAll();       
        }
    }
    ...
    const addOrder = async () => {
        let user = JSON.parse(localStorage.getItem("user"));
        let model = {userId: user._id};
        await axios.post("http://localhost:3840/orders/add", model);
        getAll();
    }
                                ...
                                <tbody>
                                    {baskets.map((basket,index)=>(
                                        <tr key={index}>
                                            ...
                                            <td>
                                                <img src={"http://localhost:3840/" + basket.products[0].imageUrl} width="75"/>
                                            </td>
                                            <td>1</td>
