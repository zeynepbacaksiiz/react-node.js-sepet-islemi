...
function ProductComponent() {
    ...
    const getAll = async () => {
        const response = await axios.get("http://localhost:3840/products")
    ...
    const remove = async (_id) => {
        ...
        if (confirm) {
            let model = { _id: _id };
            let response = await axios.post("http://localhost:3840/products/remove", model);
    ...
    const add = async (e) => {
        ...
        var response = await axios.post("http://localhost:3840/products/add", formData);
                                ...
                                <tbody>
                                    {products.map((product, index) => (
                                        <tr key={index}>
                                            <td>{index + 1}</td>
                                            <td>
                                                <img style={{ width: "50px" }} src={'http://localhost:3840/' + product.imageUrl} />
                                            </td>
                                            <td>{product.name}</td>
