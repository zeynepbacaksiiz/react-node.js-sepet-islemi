...
const uri =  "mongodb+srv://MongoDb:1@reactecommerce.ldislvu.mongodb.net/?retryWrites=true&w=majority";
...
//Basket Collection
const favori = new mongoose.Schema({
    _id: String,
    productId: String,
    userId: String,    
});

const Favori = mongoose.model("Favori", favori);
//Order Collection
...
app.post("/favori/add", async (req,res)=>{
    try {
        const {productId, userId} = req.body;
        let basket = new Favori({
            _id: uuidv4(),
            userId: userId,
            productId: productId
        });
...
app.post("/favori/getAll", async(req, res)=>{
    try {
        const {userId} = req.body;
        const favrois = await Favori.aggregate([
            {
                $match: {userId: userId}
            },
            {
